...
void ArgParserTests::generic_DND()
{
  if ((QSysInfo::kernelType() != "winnt") && !(QSysInfo::kernelType() == "darwin"))
    return;
  int i = 1;
  const int argc = 2;
  ...
  parser.setArgsBase(base);
  parser.parseGenericArgs(argc, kDragDropCmd, i);

  QVERIFY(parser.argsBase().m_enableDragDrop);
  QCOMPARE(i, 1);
}
